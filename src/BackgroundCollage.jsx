...
function cameraZToPieceProgress(cameraZ) {
  const raw = (-cameraZ - 200) / 1000 + 1;
  return Math.max(1, Math.min(22, raw));
}

function getLayerFade(progress) {
  if (progress <= 5) return 0;
  if (progress < 6) return progress - 5;
  if (progress <= 11) return 1;
  if (progress >= 22) return 0;
  return 1 - (progress - 12) / 10;
}
...
export default function BackgroundCollage({ cameraZ = -200 }) {
  ...
  const { entriesByKey } = useAmbientAudio();

  const progress = useMemo(() => cameraZToPieceProgress(cameraZ), [cameraZ]);

  useEffect(() => {
    const target = getLayerFade(progress);
    fadeRef.current = target > 0 ? Math.max(target, 0.06) : 0;
  }, [progress]);
  ...
  useEffect(() => {
    const sketch = (p) => {
      ...
      p.draw = () => {
        const layerFade = fadeRef.current;

        p.background(BG.h, BG.s, BG.b);

        if (layerFade <= 0.001) return;
        ...
        for (const c of circlesRef.current) {
          ...
          const baseAlpha = 4 + lowLevel * 55;
          const shimmer = highLevel * 22;
          const alpha = Math.min(88, (baseAlpha + shimmer) * layerFade);
  ...
  return (
    <div className={styles.backgroundCollage}>
      ...
      <div ref={mountRef} className={styles.audioLayer}></div>
