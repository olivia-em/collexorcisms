...
export default function Terminal({ onboardingDone = false }) {
  ...
  const runOliviaLoop = useCallback(() => {
    oliviaActiveRef.current = true;
    const iterate = async () => {
      if (!oliviaActiveRef.current) return;
      const acrostic = buildAcrostic(exorcismLines);
      for (const l of acrostic) {
        if (!oliviaActiveRef.current) return;
        await printLine(l, "obit-text", 200);
      }
      await printLine("", "output", 100);
      if (!oliviaActiveRef.current) return;
      const t = setTimeout(iterate, 300);
      oliviaTimersRef.current.push(t);
    };
    iterate();
  }, [exorcismLines, printLine]);
  ...
  const handleClose = useCallback(() => {
    setIsOpen(false);
    stopOliviaLoop();
    if (phase === "olivia-loop") setPhase("open");
  ...
  const handleCommand = useCallback(
      ...
      if (cmd === "obit") {
        ...
        if (args.toLowerCase() === "olivia") {
          ...
          await printLines([
            ...
            { text: "", type: "output" },
            { text: "type anything to stop", type: "hint", delay: 80 },
            { text: "", type: "output" },
          ]);
    ...
    [game, goToPiece, printLine, printLines, runOliviaLoop, stopOliviaLoop],
  );
